...
- Here at the top left you see the generation type selector and the preset you want to use. Next there is the part for your input drawing with the file selector. Further to the left you see the settings that vary according to the generation type. The part on the top right is the “preview” for generated tiles.
...
- The right part has the buttons for saving the tileset as an image or exporting to the Godot autotile tileset. Maybe other export formats will be supported later.

#### Let’s overview all the options in detail
...
#### To the drawing part. 
All the example drawings you can download separately from the program from itch.io page. Creating your own drawings can be a bit tricky since you have to keep in mind where the tiles will merge with each other. For quarters I would say it’s a lot harder to keep the tiles seamless, since in the overlay mode you don’t have to worry about the borders inside the tile and can adjust the result a bit by settings in TilePipe. 
...
Due to those limitations quarters are only good for tilesets with solid fill, like those move areas in turn-based games. For the Overlay presets you have to worry about how the sides and corners merge with the center part, but that can be adjusted from the UI. How to make the seamless merge between tiles is rather obvious here, but if it’s not, ask me in the comments or with any of the contacts provided below the video.
#### Now about the generation settings.
...
#### Now we’ve got to the output setting. Here you can
...
#### Finally we’ve got to the last part - output. 
